...
def run():
    ...
    for epoch in range(args.epochs):
        ...
        if epoch % args.evaluation_interval == 0:
            ...
            # 최고 성능 모델 저장
            if AP.mean() > best_MAP:
                checkpoint_path = result_path + '/weight/yolov3_FPGM_best.pth'.format(str(epoch + 1))
                print(f"---- Saving checkpoint to: '{checkpoint_path}' ----")
                torch.save(model.state_dict(), checkpoint_path)
                best_MAP = AP.mean()
        ...
        df.to_csv(result_path + '/graph/yolov3_FPGM.csv'.format(str(FPGM_compress_rate)), index=False)
        ...
        plt.savefig(result_path + '/graph/yolov3_FPGM_train_loss_graph.png'.format(str(FPGM_compress_rate)))
        ...
        plt.ylabel('MAP')
        plt.title('MAP_graph')
        plt.savefig(result_path + '/graph/yolov3_FPGM_test_MAP_graph.png'.format(str(FPGM_compress_rate)))
