...
def evaluate_model_file(model_path, weights_path, img_path, class_names, batch_size=8, img_size=416,
    ...
    model = load_model(model_path, weights_path, 'cuda:1')
...
def _evaluate(model, dataloader, class_names, img_size, iou_thres, conf_thres, nms_thres, verbose, device):
    ...
    print('실행 시간 : {:.4f}'.format(sum(time_list)/len(dataloader)))
...
def run():
    ...
    # parser.add_argument("-m", "--model", type=str, default="./AISOC/train_result/original_5/cfg/yolov3_predict.cfg", help="Path to model definition file (.cfg)")
    parser.add_argument("-m", "--model", type=str, default="./AISOC/config/cfg/weight_decay/0.000001/yolov3_predict.cfg", help="Path to model definition file (.cfg)")
    parser.add_argument("-w", "--weights", type=str, default="./AISOC/train_result/original_5/pruning_weight/yolov3_train_BEST.pth", help="Path to weights or checkpoint file (.weights or .pth)")
    parser.add_argument("-d", "--data", type=str, default="./data/AISOC/AISOC.data", help="Path to data config file (.data)")
