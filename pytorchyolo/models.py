...
def create_modules(module_defs):
    ...
    for module_i, module_def in enumerate(module_defs):
        ...
        # 수정
            ...
            if bn:
                modules.add_module(f"pred_batch_norm_{module_i}",
                                   nn.BatchNorm2d(filters, momentum=0.1, eps=1e-5))
            if module_def["activation"] == "leaky":
                modules.add_module(f"leaky_{module_i}", nn.LeakyReLU(0.1))
            if module_def["activation"] == "mish":
...
class Darknet(nn.Module):
    ...
    def forward(self, x):
        ...
        for i, (module_def, module) in enumerate(zip(self.module_defs, self.module_list)):
            if module_def["type"] in ["convolutional", "upsample", "maxpool", "predict"]:
                if module_def["type"] == "predict":
                    start_time = time.time()     
                x = module(x)
                if module_def['type'] == 'predict':
                    end_time = time.time()
                    self.pred_time += end_time - start_time
            elif module_def["type"] == "route":
    ...
    def load_darknet_weights(self, weights_path):
        ...
        for i, (module_def, module) in enumerate(zip(self.module_defs, self.module_list)):
            if i == cutoff:
                break
            # 수정
            if module_def["type"] == "convolutional" or module_def["type"] == "predict":
                conv_layer = module[0]
